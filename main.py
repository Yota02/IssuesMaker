...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QTabWidget, 
                             ...
                             QFileDialog, QListWidget, QMessageBox, QGroupBox,
...
class GithubIssueGeneratorGUI(QMainWindow):
    def __init__(self):
        ...
        self.token_history = self.settings.value("token_history", []) or []  # Ajout de l'historique
        ...
        for btn in [self.bug_template_btn, self.feature_template_btn, self.doc_template_btn]:
    ...
    def setup_ui(self):
        ...
        self.owner_input.setPlaceholderText("Propriétaire du dépôt (username ou organisation)")
        ...
        self.doc_template_btn.clicked.connect(lambda: self.load_template("documentation"))
        ...
        self.issues_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.Stretch)
    ...
    def verify_connection(self):
        ...
        self.issue_generator.token = token  # Utiliser token_combo au lieu de token_input
        ...
        if not self.issue_generator.owner:
            QMessageBox.warning(self, "Erreur", "Veuillez entrer le propriétaire du dépôt.")
            return
        ...
        try:
            if self.issue_generator.verify_credentials():
                ...
                QMessageBox.information(self, "Succès", "Connexion établie avec succès!")
                self.save_settings()
                ...
                self.statusBar().showMessage("Connecté à " + self.issue_generator.owner + "/" + self.issue_generator.repo)
            else:
                QMessageBox.warning(self, "Erreur", "Impossible de se connecter. Vérifiez vos informations.")
    ...
    def create_issue(self):
        ...
        if not title:
            QMessageBox.warning(self, "Erreur", "Veuillez entrer un titre pour l'issue.")
            return
        ...
        try:
            result, success = self.issue_generator.create_issue(title, body, labels, assignees)
            
            if success:
                QMessageBox.information(self, "Succès", f"Issue créée avec succès!\nURL: {result['html_url']}")
                ...
                QMessageBox.warning(self, "Erreur", f"Impossible de créer l'issue: {result}")
    ...
    def refresh_issues(self):
        ...
        try:
            ...
            if success:
                ...
                QMessageBox.warning(self, "Erreur", f"Impossible de récupérer les issues: {issues}")
                self.statusBar().showMessage("Erreur lors de la récupération des issues")
        except Exception as e:
            QMessageBox.critical(self, "Erreur", f"Une erreur est survenue: {str(e)}")
            self.statusBar().showMessage("Erreur: " + str(e))
    ...
    def edit_bulk_issue(self, item):
        ...
        if index >= 0 and index < len(self.bulk_issues):
            ...
            save_button.clicked.connect(lambda: self.save_bulk_issue(index, title_input.text(), body_input.toPlainText(), labels_input.text(), assignees_input.text(), dialog))
    ...
    def load_json_file(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(self, "Ouvrir un fichier JSON", "", "Fichiers JSON (*.json)")
        ...
        if file_path:
            try:
                ...
                self.statusBar().showMessage(f"{len(self.bulk_issues)} issues chargées depuis {file_path}")
            except Exception as e:
    ...
    def save_json_file(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(self, "Enregistrer un fichier JSON", "", "Fichiers JSON (*.json)")
        ...
        if file_path:
            try:
                ...
                self.statusBar().showMessage(f"{len(self.bulk_issues)} issues enregistrées dans {file_path}")
            except Exception as e:
                QMessageBox.critical(self, "Erreur", f"Impossible d'enregistrer le fichier: {str(e)}")
                self.statusBar().showMessage("Erreur lors de l'enregistrement du fichier")
    ...
    def create_bulk_issues(self):
        ...
        reply = QMessageBox.question(self, "Confirmation", f"Voulez-vous créer {len(self.bulk_issues)} issues?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        ...
        if reply == QMessageBox.Yes:
            ...
            try:
                results, success = self.issue_generator.create_issues_from_list(self.bulk_issues)
                
                if success:
                    QMessageBox.information(self, "Succès", f"{len(results)} issues créées avec succès!")
                    ...
                    for result in results:
                        if "error" in result:
                            error_message += f"- {result['issue']['title']}: {result['error']}\n"
                    
                    QMessageBox.warning(self, "Erreur", error_message)
                    self.statusBar().showMessage("Erreurs lors de la création des issues")
            except Exception as e:
                QMessageBox.critical(self, "Erreur", f"Une erreur est survenue: {str(e)}")
                self.statusBar().showMessage("Erreur: " + str(e))
